...
mkdir inputfiles
cp IE-0006.fasta inputfiles/"1_IE-0006.fasta"

# gunzip all files that end in .{gz,Gz,GZ,gZ}
find -L inputfiles/ -type f -name '*.[gG][zZ]' -exec gunzip -f {} +

# Filter out small inputfiles
msg "Checking input file sizes for sample 1_IE-0006.."
echo -e "Sample name	QC step	Outcome (Pass/Fail)" > "1_IE-0006.Initial_Input_File.tsv"
...
for file in inputfiles/*; do
  ...
  if verify_minimum_file_size "${file}" 'Input' "1k"; then
    echo -e "$(basename ${file%%.*})	Input File	PASS"         >> "1_IE-0006.Initial_Input_File.tsv"
  else
    echo -e "$(basename ${file%%.*})	Input File	FAIL"         >> "1_IE-0006.Initial_Input_File.tsv"
    rm ${file}
  fi
done

msg "Completed processing ${file_count} files for sample 1_IE-0006"
