...
cp "${input_file}" inputfiles/"1_ERS013358.${file_extension}${gzip_compressed}"
...
for file in inputfiles/*; do
  ...
  if verify_minimum_file_size "${file}" 'Input' "1k"; then
    ...
    echo -e "${base_name}	Input File	PASS"         >> "1_ERS013358.Initial_Input_File.tsv"
  else
    base_name=$(basename "${file%%.*}" 2>/dev/null || echo "unknown")
    echo -e "${base_name}	Input File	FAIL"         >> "1_ERS013358.Initial_Input_File.tsv"
    rm "${file}"
  fi
done
