...
cp "${input_file}" inputfiles/"1_IP-0127.${file_extension}${gzip_compressed}"

# gunzip all files that end in .{gz,Gz,GZ,gZ}
find -L inputfiles/ -type f -name '*.[gG][zZ]' -exec gunzip -f {} +

# Filter out small inputfiles
msg "Checking input file sizes for sample 1_IP-0127.."
echo -e "Sample name	QC step	Outcome (Pass/Fail)" > "1_IP-0127.Initial_Input_File.tsv"
...
for file in inputfiles/*; do
  ...
  if verify_minimum_file_size "${file}" 'Input' "1k"; then
    ...
    echo -e "${base_name}	Input File	PASS"         >> "1_IP-0127.Initial_Input_File.tsv"
  else
    base_name=$(basename "${file%%.*}" 2>/dev/null || echo "unknown")
    echo -e "${base_name}	Input File	FAIL"         >> "1_IP-0127.Initial_Input_File.tsv"
    rm "${file}"
  fi
done

msg "Completed processing ${file_count} files for sample 1_IP-0127"
