...
mkdir inputfiles
cp IP-0127.fasta inputfiles/"1_IP-0127.fasta"
...
for file in inputfiles/*; do
  ...
  if verify_minimum_file_size "${file}" 'Input' "1k"; then
    ...
    echo -e "$(basename ${file%%.*})	Input File	PASS"         >> "1_IP-0127.Initial_Input_File.tsv"
  else
    msg "DEBUG: File ${file} failed size check"
    echo -e "$(basename ${file%%.*})	Input File	FAIL"         >> "1_IP-0127.Initial_Input_File.tsv"
    rm "${file}"
  fi
done
