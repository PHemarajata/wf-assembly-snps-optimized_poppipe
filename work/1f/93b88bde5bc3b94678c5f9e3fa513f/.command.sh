...
mkdir inputfiles
cp ERS013360.fasta inputfiles/"1_ERS013360.fasta"

# gunzip all files that end in .{gz,Gz,GZ,gZ}
find -L inputfiles/ -type f -name '*.[gG][zZ]' -exec gunzip -f {} +

# Filter out small inputfiles
msg "Checking input file sizes for sample 1_ERS013360.."
echo -e "Sample name	QC step	Outcome (Pass/Fail)" > "1_ERS013360.Initial_Input_File.tsv"
...
for file in inputfiles/*; do
  ...
  if verify_minimum_file_size "${file}" 'Input' "1k"; then
    ...
    echo -e "$(basename ${file%%.*})	Input File	PASS"         >> "1_ERS013360.Initial_Input_File.tsv"
  else
    msg "DEBUG: File ${file} failed size check"
    echo -e "$(basename ${file%%.*})	Input File	FAIL"         >> "1_ERS013360.Initial_Input_File.tsv"
    rm "${file}"
  fi
done

msg "Completed processing ${file_count} files for sample 1_ERS013360"
